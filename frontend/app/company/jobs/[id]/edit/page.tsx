...
export default function EditJobPage() {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:3000/company/jobs/${jobId}?companyId=${companyId}`).then((r) => {
      ...
      setForm({
        ...
        budget: j.budget ? Number(j.budget).toString() : '',
  ...
  return (
    <div className="max-w-3xl">
      ...
      <h2 className="text-3xl font-black text-brand-black uppercase tracking-widest border-b-4 border-brand-black pb-4 mb-8">EDIT JOB</h2>
      ...
      {loaded && (
        <form onSubmit={submit} className="bg-white border-2 border-brand-black p-8 shadow-[8px_8px_0px_0px_rgba(43,43,43,1)] space-y-6">
          ...
          <div>
            ...
            <textarea rows={5} className={inputCls} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
              <label className="block text-xs font-bold uppercase tracking-widest text-brand-black mb-2">Budget (USD)</label>
              <input type="number" min="1" step="0.01" className={inputCls} value={form.budget} onChange={(e) => setForm({ ...form, budget: e.target.value })} />
            </div>
            <div>
