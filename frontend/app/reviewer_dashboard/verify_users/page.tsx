...
export default function VerifyUserModule() {
  ...
  if (loading) return <div className="text-gray-500 font-bold uppercase tracking-widest text-center mt-12">Loading pending users...</div>;
  ...
  if (users.length === 0) return (
    <div className="bg-white border-2 border-brand-black p-12 text-center shadow-[8px_8px_0px_0px_rgba(43,43,43,1)]">
      ...
      <p className="text-brand-red font-bold uppercase tracking-widest mt-4">There are no pending employee users waiting for verification.</p>
    </div>
  );

  return (
    <div className="space-y-8">
      <h2 className="text-3xl font-black text-brand-black uppercase tracking-widest border-b-4 border-brand-black pb-4">PENDING USER VERIFICATIONS</h2>
      <div className="space-y-6">
        {users.map((user) => (
          <div key={user.userId} className="bg-white p-8 border-2 border-brand-black shadow-[8px_8px_0px_0px_rgba(43,43,43,1)] flex flex-col md:flex-row justify-between md:items-start gap-6">
            <div className="flex-1">
              <h3 className="text-2xl font-black text-brand-black uppercase tracking-widest mb-4">{user.fullName || 'No Name Provided'}</h3>
              
              <div className="space-y-3 mt-6 p-4 bg-gray-50 border-l-4 border-brand-black">
                <p className="text-sm font-medium text-gray-700">
                  <strong className="text-brand-black uppercase tracking-widest text-xs inline-block w-20">EMAIL:</strong> {user.email}
                </p>
                {user.phone && (
                  <p className="text-sm font-medium text-gray-700">
                    <strong className="text-brand-black uppercase tracking-widest text-xs inline-block w-20">PHONE:</strong> {user.phone}
                  </p>
                )}
                <p className="text-sm font-medium text-gray-700">
                   <strong className="text-brand-black uppercase tracking-widest text-xs inline-block w-20">ADDRESS:</strong> {user.address || 'N/A'}
                </p>
              </div>
            </div>
