...
import { LoginDto, UpdateProfileDto, VerifyWorkDto, ChangePasswordDto, ForgotPasswordDto, ResetPasswordDto } from './reviewer.dto';
...
export class ReviewerService {
  ...
  async signup(userDto: CreateUserDto) {
    ...
    if (savedUser.role === UserRole.REVIEWER) {
      ...
      const newCompany = this.companyRepository.create({
        user: savedUser,
        companyName: savedUser.fullName,
        status: CompanyStatus.ACTIVE,
      });
      await this.companyRepository.save(newCompany);
    } else if (savedUser.role === UserRole.EMPLOYEE) {
  ...
  async forgotPassword(dto: ForgotPasswordDto) {
    ...
    otpExpiresAt.setMinutes(otpExpiresAt.getMinutes() + 15); // Valid for 15 mins
